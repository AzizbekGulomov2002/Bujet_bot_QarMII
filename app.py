    ...
    if m == 'Kodni qayta yuborish':
        if int(str(datetime.now() - baza[str(message.from_user.id)][tel_raqami]['raqam_vaqti'])[2:4]) >= 2:
            if baza[str(message.from_user.id)][tel_raqami]['qayta_yuborishlar'] <= 0:
                ...
                if a[0]:
                    await message.answer('♻ Kod qayta yuborildi. ⏱Iltimos uni 2 daqiqa ichida kiriting!')
            ...
            await message.answer('⏱ Kod yuborilganiga hali 2 daqiqa bo`lmadi iltimos kuting va qodni yuboring!')
            await Asosiy.sms_qod.set()
    elif m == 'Bekor qilish':
        
        await message.answer('❌Bekor qilindi!',reply_markup=types.ReplyKeyboardRemove())
        await message.answer(parse_mode='HTML',text='<b>📞Telefon raqamingizni kiritishingiz mumkin!\nNamuna: 996671529<b>')
        await Asosiy.tel_number.set()
    else:
